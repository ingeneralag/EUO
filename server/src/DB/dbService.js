...
export const getAll = ({ model, filter = {}, select, populate = [], skip = 0, limit = 1000 }) => {
  return Promise.all([
    model.find(filter).select(select).populate(populate).countDocuments(),
    model.find(filter).select(select).populate(populate).skip(skip).limit(limit),
  ]);
};
